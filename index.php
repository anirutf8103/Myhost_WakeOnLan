...
<body>
    ...
    <div id="pageContainer" class="_container _container-fluid">
        ...
        <header class="d-flex flex-wrap justify-content-center py-3 mb-4 border-bottom">
            <a href="/" class="d-flex align-items-center mb-3 mb-md-0 me-md-auto text-dark text-decoration-none">
                ...
                <span class="fs-4" data-lang-ckey="title">
                    <img src="./assets/x4.png" style="width:100px">
                    Wake On Lan
                </span>
                <img id="ajaxLoader"
            ...
            <ul class="nav nav-pills">
                ...
                <li class="nav-item">
                    <div class="dropdown text-end">
                        <a href="#" class="nav-link dropdown-toggle" id="dropdownTools" data-bs-toggle="dropdown"
                            aria-expanded="false" data-lang-ckey="tools">Tools</a>
                        <ul class="dropdown-menu text-small" aria-labelledby="dropdownTools">
                            <li><a id="downloadConfig" class="dropdown-item" href="?aop=CONFIG.DOWNLOAD"><i
                                        class="fa fa-file-csv"></i> <span data-lang-ckey="download_config">Download
                            ...
                            <li><a id="loadConfigFromServer" class="dropdown-item" href="#"><i
                                        class="fa fa-folder-open"></i> <span data-lang-ckey="load_config">Load
                                        Configuration</span></a></li>
                            <li><a id="saveConfigToServer" class="dropdown-item" href="#"><i class="fa fa-save"></i>
                                    <span data-lang-ckey="save_config">Save Configuration</span></a></li>
                        </ul>
                    </div>
                </li>
            </ul>
        ...
        <p style="color:red">เปิดเครื่องอัตโนมัติเวลา : <?php echo $time ?> น.</p>
        <div style="color:red;text-align: right;">
            <b>*** โปรดเปิดหน้านี้ไว้ ห้ามปิด!! ***</b>
        </div>
        ...
        <table id="hostTable" class="table table-sm table-hover">
            ...
            <tfoot>
                <tr>
                    ...
                    <td class="align-middle"><input id="mac" type="text" class="form-control" value=""
                            placeholder="Mac-Address here" data-lang-pkey="p_mac_address" /></td>
                    <td class="align-middle"><input id="host" type="text" class="form-control" value=""
                            placeholder="Ip or Hostname here" data-lang-pkey="p_ip_or_hostname" /></td>
                    <td class="align-middle"><input id="cidr" type="text" class="form-control" value=""
                            placeholder="Subnet Size here" data-lang-pkey="p_ip_subnet" /></td>
                    <td class="align-middle"><input id="port" type="text" class="form-control" value=""
                            placeholder="Port here" data-lang-pkey="p_port" /></td>
                    <td class="align-middle"><input id="comment" type="text" class="form-control" value=""
                            placeholder="Comment here" data-lang-pkey="p_comment" /></td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p data-lang-ckey="c_replace_config">Do you want to <strong>replace</strong> the existing
                        configuration?</p>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div id="importJsonHelpBlock" class="form-text">
                            To import your configuration, paste the configuration json into the field above and click
                            the <strong>[Import]</strong> button below.
                        </div>
                    </div>
    ...
    <script id="tableRowTemplate" type="text/template">
      ...
      <td class="align-middle">
        <div class="d-flex flex-row justify-content-end">
          <button type="button" id='btnWakeUpHost{{this.mac}}' class="btnWakeUpHost btn btn-outline-success btn-sm mx-1"><i class="fa fa-rocket"></i></button>
    ...
    function($) {
        ...
        if ((version[0] < 2 && version[1] < 9) || (version[0] == 1 && version[1] == 9 && version[2] < 1) || (version[
                0] > 3)) {
            throw new Error(
                'mini-i18n\'s JavaScript requires jQuery version 1.9.1 or higher, but lower than version 4. You are using version ' +
                $.fn.jquery);
        }
    }(jQuery);
    ...
    <script>
    var Time_set = "<?php echo date("Y-m-d ").$time ?>"
    var Stop_time = "<?php echo date('Y-m-d H:i:s', strtotime('+2 minutes', strtotime(date("Y-m-d ").$time))); ?>";
    </script>
    ...
    $(function() {
        ...
        $.fn.bootstrapChoice = function(options) {
            var
                defaults = {
                    modal: null,
                    onClick: function(choice, button) {
                        return false;
                    },
                    getChoice: function(button) {
                            return $(button).data('choice');
                        } // Default: return the data-choice attribute value
                        ,
                    onShow: function() {},
                    onShown: function() {},
                    onHide: function() {},
            ...
            return this.each(function() {
                ...
                $modal.on('click', 'button', function() {
                    ...
                    if (settings.onClick) closeModal = settings.onClick.apply(null, [choice,
                        this
                    ]);
    ...
    $(function() {
        ...
        var
                ...
                if (url) url.substr(1).split("&").forEach(function(item) {
                    var
                        ...
                        v = s[1] && decodeURIComponent((s[1] + '').replace(/\+/g,
                            '%20')) //  null-coalescing / short-circuit
                    ;
            ...
            renderTemplate = function(html, data, rowCallback) {
                ...
                try {
                    ...
                    console && console.error("'" + err.message + "'", " in \n\nCode:\n", code, "\n");
                }
                return result;
            }
            ...
            saveConfigToServer = function() {
                var wolConfig = $('#hostTable tbody tr').map(function() {
                    return $(this).data('wol');
                }).get();
                ...
                $.ajax({
                    ...
                    error: function(jqXHR, textStatus, errorThrown) {
                        showNotification('<small>Error ' + jqXHR.status + ' calling "' +
                            baseAddress + '": ' + jqXHR.statusText + '<hr>' + jqXHR
                            .responseText + '</small>', 'danger', 10000);
                    },
                    success: function(resp) {
                        if ('string' == typeof resp) {
                            ...
                            makeClean();
                            showNotification($('#textConfigSavedSucessfully').html(), 'success',
                                3000);
                        }
                    }
                });
                    ...
                    success: function(resp) {
                        if ('string' == typeof resp) {
                            ...
                            for (var i = 0; i < resp.length; i++) {
                                addHost(resp[i].mac, resp[i].host, resp[i].cidr, resp[i].port, resp[
                                    i].comment);
                            }
                ...
                if (0 == $tr.length) {
                    ...
                    $.ajax({
                        ...
                        beforeSend: function( /* xhr */ ) {
                            $i
                                .removeClass(
                                    'fa-question fa-eye fa-thumbs-up fa-thumbs-down text-danger text-success'
                                )
        ...
        $('#importJsonConfig').on('click', function() {
            ...
            for (var i = 0; i < config.length; i++) {
                addHost(config[i].mac, config[i].host, config[i].cidr, config[i].port, config[i]
                    .comment);
            }
        ...
        function checktime() {
            ...
            var date2 = new Date(Time_set);
            var datetime_stop = new Date(Stop_time);

            // console.log(date2);
            // console.log(datetime_stop);

            if (date1 >= date2) {
                if (date1 <= datetime_stop) {
                    autoClick();
                }
            }
        }
            ...
            $.ajax({
                ...
                success: function(resp) {
                    if ('string' == typeof resp) {
                        ...
                        for (var i = 0; i < resp.length; i++) {
                            console.log(resp[i].mac);
                            document.getElementById("btnWakeUpHost" + resp[i].mac).click();
                        }
                        if (!doNotMakeClean) makeClean();
                    }
                }
